...
export function AuctionProductView() {
  const t = useTranslation();
  const { language } = useLanguage();
  const { product, isLiked, isOwner, isDeleting, handleLike, startEditing, handleDelete } = useAuctionDetail();

  const { data: images } = useQuery<ImageUploadResponse[]>({
    queryKey: ['auction', 'product', 'images', product?.id, product?.image_ids ?? []],
    queryFn: async () => {
  ...
  return (
    <>
      ...
      {/* Sold Badge and Translate Button */}
